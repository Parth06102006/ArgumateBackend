...
const sentimentAnalysis = asyncHandler(async(req,res)=>{
    ...
    try {
        ...
        return res.status(200).json(new ApiResponse(200,'SentimentAnalysis : HTML Content Retrieved Successfully',{html:`
        ...
        throw new ApiError(500,'Unable to Generate Sentiment Analysis')
    }
});

const emotionAnalysis = asyncHandler(async(req,res)=>{
    const {speech} = req.body;
    const prompt = `
    ...
    try {
        ...
        return res.status(200).json(new ApiResponse(200,'EmotionalAnalysis : HTML Content Retrieved Successfully',{html:`
            <div style="font-family: Arial, sans-serif; line-height: 1.6;">
            <strong>Dominant Emotion:</strong> ${json.dominantEmotion}<br>
        ...
        console.error(error.messsage)
        throw new ApiError(500,'Unable to Generate Emotional Analysis')
    }
});
